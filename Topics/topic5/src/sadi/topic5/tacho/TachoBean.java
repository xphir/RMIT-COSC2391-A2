...
public class TachoBean extends JComponent
{
	...
	private GradientPaint gradientPainter = new GradientPaint(0, 0,
			Color.black, 255, 255, Color.gray, true);

	public TachoBean()
	{
		this(8000, 1000, 500, 100, 110, 270, 360, 10, 10, 6, 16, 500, 1000,
				6500);
	}

	public TachoBean(int maxRPM, int majorRPMIncrement, int minorRPMIncrement,
			int needleRadius, int gaugeRadius, int needleSweepDegrees,
			int gaugeDegrees, int needleBasetrailPixels, int needleTipPixels,
			int needleThicknessPixels, int gaugeBodyThicknessPixels,
			int minorGraduationValue, int majorGraduationValue, int redline)
	{
	...
	GeneralPath getNeedlePolygon2D(int x, int y)
	{
		// lineTo points for needle polygon
		int xPoints[] =
		{ x + getNeedleBasetrailPixels(),
				x - (getNeedleRadius() - getNeedleTipPixels()),
				x - getNeedleRadius(),
				x - (getNeedleRadius() - getNeedleTipPixels()),
				x + getNeedleBasetrailPixels() };
		int yPoints[] =
		{ y + (getNeedleThicknessPixels() / 2),
				y + (getNeedleThicknessPixels() / 2), y,
				y - (getNeedleThicknessPixels() / 2),
				y - (getNeedleThicknessPixels() / 2) };
	...
	public void drawGaugeFace(Graphics2D g2D)
	{
		...
		Arc2D.Double gaugeFace = new Arc2D.Double(x1, y1, x2, y2, 0.0,
				(double) getGaugeDegrees(), Arc2D.Double.OPEN);
		g2D.draw(gaugeFace);
	...
	public void drawGaugeGraduations(Graphics2D g2D)
	{
		...
		if (getMajorGraduationValue() != -1)
		{
			...
			{
				theta = (double) i / getMaxRPM()
						* (getNeedleSweepDegrees() / 180.0 * Math.PI);
				xOuter = (int) Math.round((double) originX
						- (r * Math.cos(theta)));
				yOuter = (int) Math.round((double) originY
						- (r * Math.sin(theta)));

				xInner = (int) Math.round((double) originX
						- (0.90 * r * Math.cos(theta)));
				yInner = (int) Math.round((double) originY
						- (0.90 * r * Math.sin(theta)));
		...
		if (getMinorGraduationValue() != -1)
		{
			for (int i = 0; i <= getMaxRPM(); i += getMinorGraduationValue())
			{
				...
				if ((i % getMajorGraduationValue()) != 0)
				{
					...
					xInner = (int) Math.round((double) originX
							- (0.95 * r * Math.cos(theta)));
					yInner = (int) Math.round((double) originY
							- (0.95 * r * Math.sin(theta)));
	...
	private void drawNewNeedle(Graphics2D g2D)
	{
		...
		double theta = (double) rpmValue / (double) getMaxRPM()
				* ((double) getNeedleSweepDegrees() / 180.0 * Math.PI);
	...
	private TexturePaint createTiledImagePainter(String imageFile)
	{
		...
		BufferedImage bufferedImage = new BufferedImage(imageHeight,
				imageHeight, BufferedImage.TYPE_INT_RGB);
		Graphics2D g2d = bufferedImage.createGraphics();
		g2d.drawImage(image, 0, 0, this);

		return new TexturePaint(bufferedImage, new Rectangle(0, 0, imageWidth,
				imageHeight));
	}
